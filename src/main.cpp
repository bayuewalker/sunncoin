...
#if defined(NDEBUG)
# error "Sunncoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0xdd3e4feba238f46171f0aebcd05bf990753a9d3a6954fd490ffebcd435df394e");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Sunncoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Sunncoin Signed Message:\n";
...
{
    // Sunncoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 100000); // Sunncoin: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 1 * 24 * 60 * 60; // Sunncoin: 3.5 days
static const int64 nTargetSpacing = 5 * 60; // Sunncoin: 2.5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Sunncoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Sunncoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x6c42b62207d3980adcc5abca110649379db12a01893a175bb5ca10facf57eccd");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "Doug Jones defeats Roy Moore in Alabama senate election 2018";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime    = 1537774344;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2085184572;

        if (fTestNet)
        {
            block.nTime    = 1537774356;
            block.nNonce   = 389015921;
        }
...
if (false && block.GetHash() != hashGenesisBlock)
        ...
        assert(block.hashMerkleRoot == uint256("0x822cfa79a45600a6c3d67978d07147261385256e6c1b652121f6d43687b4f126"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xf1, 0xca, 0xb2, 0xd3 }; // Sunncoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("SunncoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("SunncoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static SunncoinMiner(CWallet *pwallet)
{
    printf("SunncoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("sunncoin-miner");
    ...
    try { loop {
        ...
        printf("Running SunncoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("SunncoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&SunncoinMiner, pwallet));
}
